...
def place_face_on_canvas(face_img, canvas_w, canvas_h, bg_color=(255, 255, 255)):
    ...
    scale_w = canvas_w / w
    scale_h = canvas_h / h
    scale = min(scale_w, scale_h)  # 选择较小的缩放比例，确保完全放入
    ...
    # 创建画布
    canvas = np.ones((canvas_h, canvas_w, 3), dtype=face_img.dtype) * np.array(bg_color, dtype=face_img.dtype)
...
def process_folder(folder_path, detector, max_size, resize, threshold, output_folder='examples'):
    ...
    for date in sorted_dates:
        ...
        for img_info in date_groups[date]:
            ...
            if len(faces) > 1:
                ...
                print(f"    检测到 {len(faces)} 个人脸，只处理置信度最高的 (score: {best_face['score']:.3f})")
            else:
                best_face = faces[0]
                print(f"    检测到 1 个人脸 (score: {best_face['score']:.3f})")
            
            # 计算 yaw 角度
            yaw = calculate_yaw_angle(best_face['kps'])
            
            # 截取放大后的人脸区域
            face_crop, expanded_box = crop_face_with_expansion(img, best_face['box'], scale=1.5)
            
            date_face_data.append({
                'image': face_crop,
                'yaw': yaw,
            ...
            print(f"    Yaw角度: {yaw:.2f}° {'(向左)' if yaw > 0 else '(向右)' if yaw < 0 else '(正中间)'}")
            print(f"    截取区域大小: {face_crop.shape[1]}x{face_crop.shape[0]}")
        
        if len(date_face_data) == 0:
            print(f"  日期 {date} 没有有效的人脸数据，跳过")
            continue
        
        # 按照 yaw 角度降序排序（偏左最大的在最左边）
        date_face_data.sort(key=lambda x: x['yaw'], reverse=True)
        
        print(f"\n  按 Yaw 角度排序后的顺序:")
        for i, data in enumerate(date_face_data):
            print(f"    {i+1}. {data['filename']} - Yaw: {data['yaw']:.2f}°")
        
        # 保存该日期的人脸数据
        all_faces_by_date[date] = date_face_data

    # 检查是否有有效的日期数据
    if len(all_faces_by_date) == 0:
        print(f"\n❌ 文件夹 {folder_name} 中没有任何有效的日期数据可以拼接")
        return None
    ...
    # 找到所有人脸中最大的宽度和高度
    max_width = max(f['image'].shape[1] for f in all_faces_list)
    max_height = max(f['image'].shape[0] for f in all_faces_list)

    # 画布尺寸设为最大宽度和最大高度（确保所有人脸都能放入）
    canvas_width = max_width
    canvas_height = max_height

    print(f"所有人脸的尺寸范围:")
    print(f"  宽度: {min(f['image'].shape[1] for f in all_faces_list)} ~ {max_width}")
    print(f"  高度: {min(f['image'].shape[0] for f in all_faces_list)} ~ {max_height}")
    print(f"统一画布尺寸: {canvas_width}x{canvas_height}")

    # 第三步：将所有人脸按比例缩放并居中放置在统一画布上
    print(f"\n{'='*60}")
